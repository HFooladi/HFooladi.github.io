...
- Publication thumbnails: set `header.teaser`, convention `assets/images/publications/<slug>.png|jpg` (TOC/graphical-abstract figures work well); they are shown whole (`object-fit: contain` on white), not cropped, so figure text stays legible. Without one a gradient venue tile renders
